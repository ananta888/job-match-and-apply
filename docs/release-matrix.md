...
| Claude Code | `supported`, **exakt `2.1.232`–`2.1.234 (Claude Code)`**, Adapter `1.1.0` | ausschließlich WSL mit Bubblewrap | read-only, Safe Mode, Permission Mode `acceptEdits`; Provider-Control-Plane bleibt erreichbar | prompt-only; leerer MCP-Vertrag, keine Root-Tools, keine interaktive Approval-, Pause- oder Resume-Brücke | andere Versionen/Runtimes werden blockiert; im Profil deaktivieren oder wieder eine gepinnte `2.1.23x`-Version bereitstellen |
| Agent Client Protocol (`acp`) | `experimental`, **nur `acp-synthetic 0.1.0`**, Adapter `0.1.0` | Windows, Linux, macOS; nicht WSL/Container | JSON-RPC 2.0/NDJSON stdio; Client ohne `fs/*` und `terminal/*`; `mcpServers: []`; nur read-only und `network: disabled` | prompt-only; keine Root-Tools, kein CV-KI-Zero-Tools-Vertrag, keine Approval-/Resume-Brücke | nur bei ausdrücklichem `provider=acp`; reale Binaries bleiben untested/blockiert; im Profil deaktivieren |
...
| Evidence-Pipeline | `evidence-application-package@1.1.0`, fünf getrennte Runs: Evidence → Author → ATS/Recruiter-Style → Finalizer | kein Browser-Zwischengate; beide Reviews gehen automatisch an den Finalizer; keine automatische externe Aktion | Orchestrierung revisionsgebunden abbrechen; kein Teilergebnis als finale HTML-Seite ausgeben |
| `final_html` | direktes, normalisiertes HTML5-Ergebnis des fünften Agenten | exakt eine content-addressed `application-final-html`-Datei; hashgebundene `result.html`-Route, CSP und iframe-Sandbox; Anzeige ohne Review/Freigabe | Lauf abbrechen oder Ergebnis nicht extern verwenden; alte `package_proposal` bleibt nur Legacy-Kompatibilität |
...
|---|---|---|---|---|
| `bewerbungs-schreib-assistent` | `integrations/bewerbungs-schreib-assistent` | `cb8459849431046c5405c93eb1cee96dccd7d75f` | Application Pipeline `1.x` über lokale CLI/Artefakte; Candidate Evidence bleibt autoritativ | Root-Commit mit vorherigem Gitlink auschecken; keine Dateien aus dem Submodule ins Root kopieren |
